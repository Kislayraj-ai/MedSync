...
(function () {
  ...
  //   try {
  //     const response = await fetch(`${BASE_URL}/api/v1/revenue-data/`);
...
const currentYear = new Date().getFullYear(); 
    ...
    // Helper to map API months to 12 months array
    function mapYearData(apiData, year) {
      const values = Array(12).fill(0); // default 0 for missing months
      apiData.totalRevenue.months.forEach((monthStr, index) => {
        const [monthAbbr, yr] = monthStr.split(' ');
        if (parseInt(yr) === year) {
          const monthIndex = allMonths.indexOf(monthAbbr);
          if (monthIndex > -1) values[monthIndex] = apiData.totalRevenue.values[index];
        }
      });
      return values;
    }
    ...
    const series = [
      {
        name: currentYear,
        data: mapYearData(data, currentYear)
      },
      {
        name: previousYear,
        data: mapYearData(data, previousYear )
      }
    ];

    // Responsive breakpoints copied from your original design
    ...
    const totalRevenueChartOptions = {
      ...
      fill: { opacity: [1, 1] },
      xaxis: { categories: allMonths, labels: { style: { fontSize: '13px', fontFamily: 'Public Sans', colors: labelColor } }, axisTicks: { show: false }, axisBorder: { show: false } },
      yaxis: { labels: { style: { fontSize: '13px', fontFamily: 'Public Sans', colors: labelColor } } },
      responsive: responsiveConfig // fixed here
    };
